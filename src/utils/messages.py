...
def default_embed(
    user: discord.Member, title: str, color: str = None, width: int = 25, **kwargs
) -> discord.Embed:
    """Create a default embed."""

    embed = discord.Embed(
        color=color or random_color(), title=title.ljust(width, "\u2800"), **kwargs
    )
...
async def select_amount(
    ...
    if done and not isinstance(
        (interaction := done.pop().result()), discord.Interaction
    ):
